    ...
    def _update_conditions_data(self):
        ...
        def run_query(lat=lat, lon=lon, name=name):
            try:
                ...
                err_msg = str(e)
                self.root.after(0, lambda: messagebox.showerror("Erro", f"Falha ao carregar condições: {err_msg}"))
        ...
        def run_search(lat=lat, lon=lon, query_formatada=query_formatada):
            try:
                ...
                matched = [
                    ...
                    if query_formatada.lower() in obj.name.lower()
                ]

                if matched:
                    icon_map = {
                        "Lua": "🌕", "Mercúrio": "☿️", "Vênus": "♀️",
                    ...
                    self.root.after(0, lambda: messagebox.showinfo("Busca", f"Objeto '{query_formatada}' não encontrado nos catálogos."))

            except Exception as e:
                err_msg = str(e)
                self.root.after(0, lambda: messagebox.showerror("Erro", f"Falha na busca: {err_msg}"))
    ...
    def _on_object_select(self, item):
        ...
        def run_query(lat=lat, lon=lon, name=name, item=item):
            try:
                ...
                selected_planet = [
                    p for p in result.planets if p.name == item["name"]
                ]
                ...
                err_msg = str(e)
                self.root.after(0, lambda: messagebox.showerror("Erro", f"Falha ao carregar dados: {err_msg}"))
            ...
            try:
                ...
                if result:
                    ...
                    def do_register(name=loc_name, lat=loc_lat, lon=loc_lon, country=loc_country):
                        ...
                        messagebox.showinfo("Sucesso", f"Local '{name}' ({country}) salvo com sucesso!")
                    ...
                    self.root.after(0, lambda: messagebox.showerror("Erro", f"Não foi possível encontrar a cidade '{term}'."))
            except Exception as e:
                err_msg = str(e)
                self.root.after(0, lambda: messagebox.showerror("Erro", f"Falha ao adicionar local: {err_msg}"))
