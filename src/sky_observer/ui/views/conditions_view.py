...
class ConditionsView(tk.Frame):
    ...
    def _create_semaphore_card(self, parent):
        ...
        circle_canvas = tk.Canvas(card, width=70, height=70, bg=COLORS["bg_primary"], highlightthickness=0)
        circle_canvas.pack(side=tk.LEFT, padx=(0, 16))
        
        self.widgets["sem_canvas"] = circle_canvas
        self.widgets["sem_oval"] = circle_canvas.create_oval(4, 4, 66, 66, fill=COLORS["bg_secondary"], outline=COLORS["border"], width=2)
        self.widgets["sem_icon"] = circle_canvas.create_text(35, 35, text="...", font=("Helvetica", 22, "bold"), fill=COLORS["text_muted"])
    ...
    def _planet_row(self, parent, name, detail, status):
        ...
        dot = tk.Canvas(row, width=10, height=10, bg=COLORS["bg_primary"], highlightthickness=0)
    ...
    def update_display(self, data: dict):
        ...
        if "is_object_context" in data:
            if data["is_object_context"]:
                self.widgets["btn_back"].pack(side=tk.LEFT, padx=(0, 12), before=self.widgets["main_title"])
        ...
        if "status" in data:
            ...
            if st == "good":
                canvas.itemconfig(self.widgets["sem_oval"], fill=COLORS["green_bg"], outline=COLORS["green"])
                canvas.itemconfig(self.widgets["sem_icon"], text="✓", fill=COLORS["green"])
                self.widgets["score_val"].config(fg=COLORS["green"])
            elif st == "fair":
                canvas.itemconfig(self.widgets["sem_oval"], fill=COLORS["yellow_bg"], outline=COLORS["yellow"])
                canvas.itemconfig(self.widgets["sem_icon"], text="!", fill=COLORS["yellow"])
                self.widgets["score_val"].config(fg=COLORS["yellow"])
            elif st == "bad":
                canvas.itemconfig(self.widgets["sem_oval"], fill=COLORS["red_bg"], outline=COLORS["red"])
    ...
    def _load_dummy_data(self):
        ...
        self.update_display({
            ...
            "planets": [
                ...
                {"name": "Júpiter", "detail": "Alt. 44°  ·  Az. 210°  ·  21h00 – 03h10", "status": "green"},
                {"name": "Marte",   "detail": "Alt. 18°  ·  Az. 270°  ·  04h00 – 05h40", "status": "yellow"},
